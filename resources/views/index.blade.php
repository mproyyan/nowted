        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
          ...
          <ul class="space-y-3 sm:flex sm:space-y-0 sm:space-x-8 md:space-x-12">
            <a href="{{ route('favorites') }}" class="block">
              <li class="{{ request()->is('favorites') ? 'text-white' : '' }} flex items-center sm:hover:text-white md:font-semibold">
            ...
            <a href="{{ route('archives') }}" class="block">
              <li class="{{ request()->is('archives') ? 'text-white' : '' }} flex items-center sm:hover:text-white md:font-semibold">
                <svg class="mr-2 w-5 sm:hidden" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <a href="{{ route('trash') }}" class="block">
              <li class="{{ request()->is('trash') ? 'text-white' : '' }} flex items-center sm:hover:text-white md:font-semibold">
                <svg class="mr-2 w-5 sm:hidden" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @auth
              <li x-data="{ open: false }" @click="open = !open" class="flex cursor-pointer items-center sm:hover:text-white md:font-semibold">
                <svg class="mr-2 w-5 sm:hidden" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <template x-teleport="body">
                  <x-modal :url="route('logout')" title="Logout From Account" confirm="Logout">
                    ...
                    <h4 class="mt-2 text-center text-lg text-gray-300">Are you sure that you want to <span class="text-white font-semibold">logout</span>?</h4>
                    <p class="mt-2 text-center text-sm text-gray-400">Once you log out, you will be asked to fill in your credentials again when accessing a restricted page</p>
                  </x-modal>
                </template>
              </li>
            @endauth
